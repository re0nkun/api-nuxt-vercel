  ...
  <div>
    ...
    >
      <v-list-item class="pl-2 min-height-0">
        <a :href="tw_main_url" target="_blank" rel="noopener noreferrer">
          <v-list-item-avatar size="28" class="ma-0">
            <v-img :src="tw.user.profile_image_url" />
          </v-list-item-avatar>
        </a>

        <v-list-item-content class="ml-1 pa-1">
          <v-list-item-title>
            {{ tw.user.name }}<span class="subtitle-2 font-weight-light pl-2">@{{ tw.user.username }}</span>
          </v-list-item-title>
        </v-list-item-content>
      </v-list-item>
      ...
      <v-card-text class="card-text pa-2">
        <!-- <div v-if="fullText" class="full-text brown lighten-5">
          ...
          <a id="card-link" :href="tw_url" target="_blank" rel="noopener noreferrer">
          {{ tw.text }} ({{ tw.created_at }})
          </a>
        </div> -->
      ...
      <v-responsive :aspect-ratio="ratio">
        <div>
          <a id="card-link" :href="tw_url" target="_blank" rel="noopener noreferrer">
            <span v-if="tw.media[0].type === 'photo'">
              <v-img
                :src="tw.media[0].url"
                :aspect-ratio="ratio"
              />
            </span>
            <span v-else-if="tw.media[0].type === 'video'">
              <v-img
                :src="tw.media[0].preview_image_url"
                :aspect-ratio="ratio"
              />
            </span>
          </a>
...
export default {
  ...
  data () {
    return {
      tw_url: `https://twitter.com/${this.tw.user.username}/status/${this.tw.id}`,
      tw_main_url: `https://twitter.com/${this.tw.user.username}`,
  ...
  computed: {
    ratio () {
      return this.tw.media[0].width / this.tw.media[0].height
    }
  },
  methods: {
    // altSrc () {
